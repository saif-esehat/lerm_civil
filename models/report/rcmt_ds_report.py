...
class RcmtDatasheet(models.AbstractModel):
        _name = 'report.lerm_civil.rcmt_datasheet'
        _description = 'RCMT DataSheet'
...
class RCMTReport(models.AbstractModel):
    _name = 'report.lerm_civil.rcmt_mec_report'
    ...
    def _get_report_values(self, docids, data):
        ...
        rcmt_data = self.env[model].sudo().search([("id","=",eln.model_id)])
        return {
            'eln': eln,
            'rcmt': rcmt_data,
            'qrcode': qr_code
