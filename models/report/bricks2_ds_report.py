...
class BrickDatasheet(models.AbstractModel):
    _name = 'report.lerm_civil.lerm_brick_datasheet'
...
class BrickReport(models.AbstractModel):
    _name = 'report.lerm_civil.lerm_brick_report'
    ...
    def _get_report_values(self, docids, data):
        ...
        brick_data = self.env[model].sudo().search([("id","=",eln.model_id)])
        return {
            'eln': eln,
            'brick': brick_data,
            'qrcode': qr_code,
